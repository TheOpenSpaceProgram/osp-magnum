...
The OSP-Magnum game engine needs to be driven by a full application, aka game, which uses the game engine to provide the gameplay, and is responsible and in full control of every feature. The OSP-Magnum engine only provides building blocks.
...
* **Extensive use of contiguous containers**. Dealing with streams of data is the common case. Contiguous containers offer best performance with O(1) access and perfect cache locality. Can't really say `std::vector` is the best in all situations, but it is heavily used.
* **No runtime Polymorphism**. We don't need polymorphism. Instead, prefer a more true form of message passing through shared variables. E.g. Two completely different systems can communicate using queues that act as interfaces.
...
* **Component**: An (optional) property of an entity. Usually a struct stored in a contiguous container that maps a component back to a specific Entity.
...
As another example, a `Rocky Planet` can be represented by an Entity having a `Mass`, `Radius`, `Atmosphere`, and `Solid surface` component. A `Star` can share the same `Mass`, `Radius`, and `Atmosphere`, but with a `Luminance` and `Plasma surface` component. A `Gas Giant` planet would have `Mass`, `Radius`, `Atmosphere`, and `Gas Surface`.
...
Systems are called in a specific order to update the entire world to the next frame. Some systems may depend on each other, and other systems can run in parallel on multiple CPU cores. The ordering of the systems is not the responsibility of the ECS system, but is instead the responsibility of the application that uses the ECS system.

The EnTT library provides many features making iteration convenient.

### Responsibility of Systems

Ideally, components are raw data that don't describe any behaviour. However, it's acceptable for them to have constructors, destructors, member functions, and RAII members as long as components are only modified by very specific systems. This includes creating and destroying components. Many systems reading the same component simultaniously on multiple CPUs is OK. The only conflict is writing to a component at the same time as another thread is using the component (read or write).
...
In short, **components must only be modified by a few select systems**, and the top level application must be designed to know which systems use which components, globally. There's a few other practical reasons why:
...
* Multiple Systems that modify unrelated components can run in parallel without mutexes.

There's a few important operations that need additional rules however:
